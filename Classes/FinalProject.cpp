...
void FinalProject::setPhysicsWorld(PhysicsWorld* world) { m_world = world; }

int FinalProject::which = 4;
bool FinalProject::IsFoodEaten = false;
bool FinalProject::IsTeaDrunk = false;
Scene* FinalProject::createScene() {
	...
	auto layer = FinalProject::create();
...
bool FinalProject::init() {
	...
	addPlayer();    // 添加玩家
	addFloor();		// 添加场景
	schedule(schedule_selector(FinalProject::update), 0.01f, kRepeatForever, 0.1f);
	isRotate = false;
	return true;
}

// 预加载音效
void FinalProject::preloadMusic() {
...
void FinalProject::addTrap(int x, int y, bool flag, int xx, int yy, int tag) {
...
void FinalProject::addFood(int x, int y, int type, int xx, int yy, int tag) {
	auto yLen = visibleSize.height / 19;
	auto xLen = visibleSize.width / 25 / 1.4;
	Sprite* h;
	if (type == 0) {
...
void FinalProject::addHorizontal(int x, int y) {
	auto yLen = visibleSize.height / 19;
	auto xLen = visibleSize.width / 25 / 1.4;
...
void FinalProject::addVertical(int x, int y) {
	auto yLen = visibleSize.height / 19;
	auto xLen = visibleSize.width / 25 / 1.4;
...
void FinalProject::addBlock(int x, int y) {
	auto yLen = visibleSize.height / 19;
	auto xLen = visibleSize.width / 25 / 1.4;
...
void FinalProject::addCheckpoint(int x, int y, int which) {
	auto yLen = visibleSize.height / 19;
	auto xLen = visibleSize.width / 25 / 1.4;
...
void FinalProject::addTransparent(int x, int y, int xx, int yy, int tag) {
	auto yLen = visibleSize.height / 19;
	auto xLen = visibleSize.width / 25 / 1.4;
...
// 添加监听器
void FinalProject::addListener() {
	auto keyboardListener = EventListenerKeyboard::create();
	keyboardListener->onKeyPressed = CC_CALLBACK_2(FinalProject::onKeyPressed, this);
	keyboardListener->onKeyReleased = CC_CALLBACK_2(FinalProject::onKeyReleased, this);
	_eventDispatcher->addEventListenerWithSceneGraphPriority(keyboardListener, this);
	auto contactListener = EventListenerPhysicsContact::create();
	contactListener->onContactBegin = CC_CALLBACK_1(FinalProject::onContactBegin, this);
	_eventDispatcher->addEventListenerWithSceneGraphPriority(contactListener, this);
}

bool FinalProject::onContactBegin(PhysicsContact & contact) {
...
//加载玩家动画帧, 共8种 :)
void FinalProject::loadAnimation(string filepath) {
...
// 上左右 
void FinalProject::onKeyPressed(EventKeyboard::KeyCode code, Event* event) {
	if (code == cocos2d::EventKeyboard::KeyCode::KEY_R) {
		Director::getInstance()->replaceScene(FinalProject::createScene());
	}
...
// 释放按键
void FinalProject::onKeyReleased(EventKeyboard::KeyCode code, Event* event) {
	if (IsMoveEnable) {
...
// 游戏结束
void FinalProject::GameOver() {
	IsMoveEnable = false;
	unschedule(schedule_selector(FinalProject::update));
	...
	auto replayBtn = MenuItemLabel::create(label2, CC_CALLBACK_1(FinalProject::replayCallback, this));
...
// 继续或重玩按钮响应函数
void FinalProject::replayCallback(Ref * pSender) {
	Director::getInstance()->replaceScene(FinalProject::createScene());
}

// 退出
void FinalProject::exitCallback(Ref * pSender) {
	Director::getInstance()->end();
#if (CC_TARGET_PLATFORM == CC_PLATFORM_IOS)
	exit(0);
#endif
}
void FinalProject::addFloor() {
	//add floor
